...
def request_more_unique_titles(query, request_size, set_of_existed_titles, number_of_needed_titles):
    ...
    while (number_of_try > 0 and len(additional_list) < number_of_needed_titles):
        got_request = make_request(query, 5000)
...
def create_list_of_unigue_answers(input_dir = '/notebook/touche/', input_file = 'topics-task-2-only-titles.xml', list_of_tuples = None):
    ...
    try:
        for elem in list_of_tuples:
            ...
            if (len(common_list + add_list) < 1000):
                print ("len(vocabulary_of_all_answers[qid]) < 1000", len(common_list + add_list))
                vocabulary_of_all_answers[qid] = common_list + add_list + common_list_reserve[:(1000 - len(common_list + add_list))]
                print ("len(vocabulary_of_all_answers[qid]) < 1000", len(vocabulary_of_all_answers[qid]))
    except:
